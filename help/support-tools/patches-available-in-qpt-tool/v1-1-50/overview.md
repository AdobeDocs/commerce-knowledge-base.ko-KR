...
1. **ACSD-45049**: 관리자의 웹 사이트 범위에 따라 고객 특성 *[!UICONTROL Is required]* 설정이 제대로 작동하지 않는 문제를 해결했습니다.
...
1. **ACSD-54887**: 지속적인 장바구니를 사용하도록 설정한 상태에서 고객 세션이 만료된 후 고객 장바구니가 지워지는 문제를 해결했습니다.
...
1. **ACSD-58442**: 너비가 768px인 장치가 모바일로 처리되어 메뉴 및 헤더가 데스크탑 대신 모바일 보기에서 로드되는 문제를 해결했습니다.
1. **ACSD-58790**: [!DNL Chrome]의 모바일 보기에서 제품 세부 사항 페이지 이미지에 대한 *핀치-투-줌* 기능을 수정합니다.
1. **ACSD-59036**: 하한과 상한이 모두 $0인 제품 가격을 로드할 때 발생하는 예외를 수정합니다.
1. **ACSD-59229**: 요청에서 X-Magento-Vary의 이전 값으로 인해 고객 그룹 관련 정보가 잘못된 세그먼트에 저장되는 문제가 해결되었습니다.
