...
MDVA-34023 패치는 고객의 웹 브라우저에서 `No such entity with addressId` 오류가 임의로 발생하는 문제를 해결합니다.
...
**Adobe Commerce 버전용 패치가 만들어졌습니다.** 클라우드 인프라 2.3.1의 Adobe Commerce
...
1. **스토어** > **설정** > **구성** > **고객 탭** > **지속적인 장바구니**&#x200B;로 이동합니다.
1. **지속성 사용** = *예*, **로그아웃 시 지속성 지우기** = *아니요*&#x200B;를 설정합니다.    ![persistent_shopping_cart_magento_2.4.1.png](/help/support-tools/patches-available-in-qpt-tool/assets/persistent_shopping_cart_magento_2.4.1.png)
1. 새 고객을 만들고 기본 배송 및 청구 주소를 정의합니다.
1. 로그아웃.
1. **내 정보 저장** 확인란을 선택하여 로그인합니다.
1. `customer_entity` DB 테이블로 이동하여 `default_billing` 및 `default_shipping` ID를 존재하지 않는 ID로 변경합니다.
1. 로그아웃.
