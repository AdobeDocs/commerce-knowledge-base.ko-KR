...
`.magento.app.yaml`에 잘못된 Nginx 구성 집합이 있기 때문에 문제가 발생합니다. `index.php` 대신 `"/media"` 위치에 `passthru: /get.php`(또는 없음)을(를) 사용합니다.

## 솔루션

1. `.magento.app.yaml` 위치에서 `"/media"` 구성 파일을 확인하십시오. 올바른 구성은 다음과 같습니다.
...
1. 구성 파일을 수정합니다.
   ...
   * 스테이징 및 프로덕션: 파일을 직접 수정하고 변경 내용을 푸시한 다음 [Adobe Commerce 지원 티켓](https://experienceleague.adobe.com/ko/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide)을 만들어 적용합니다.
...
구성이 올바르지만 문제가 계속 발생하면 조사를 계속하거나 [Adobe Commerce 지원](https://experienceleague.adobe.com/ko/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide)에 문의하십시오.
