...
1. 개발자 설명서에서 [[!UICONTROL indexers]](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/manage-indexers)이(가) 실행 중이고 ```php    Product Feed    ``` [!UICONTROL indexer]이(가) ```php    Update by Schedule    ```(으)로 설정되어 있는지 확인하십시오.
